...
static int transcode_init(void)
{
    ...
    av_log(NULL, AV_LOG_INFO, "nb_filtergraphs\n");
    for (i = 0; i < nb_filtergraphs; i++) {
        FilterGraph *fg = filtergraphs[i];
        for (j = 0; j < fg->nb_outputs; j++) {
    ...
    av_log(NULL, AV_LOG_INFO, "init framerate emulation\n");
    ...
    av_log(NULL, AV_LOG_INFO, "for each output stream, we compute the right encoding parameters\n");
...
//                    ost->frame_rate = ist->st->avg_frame_rate.num ? ist->st->avg_frame_rate : (AVRational){25, 1};
    ...
    av_log(NULL, AV_LOG_INFO, "open each encoder\n");
    /* open each encoder */
    for (i = 0; i < nb_output_streams; i++) {
    ...
    av_log(NULL, AV_LOG_INFO, "init input streams\n");
    /* init input streams */
    for (i = 0; i < nb_input_streams; i++) {
    ...
    av_log(NULL, AV_LOG_INFO, "discard unused programs\n");
    ...
    av_log(NULL, AV_LOG_INFO, "open files and write file headers\n");
...
//         assert_avoptions(output_files[i]->opts);
 ...
 dump_format:
    av_log(NULL, AV_LOG_INFO, "dump the file output parameters - cannot be done before in case of stream copy\n");
    ...
    av_log(NULL, AV_LOG_INFO, "for nb_output_streams\n");
    for (i = 0; i < nb_output_streams; i++) {
        ost = output_streams[i];
        ...
        if (ost->stream_copy)
            ...
            if (out_codec) {
                ...
                av_log(NULL, AV_LOG_INFO, " out_codec_name: %s ", out_codec_name);
                if (!strcmp(encoder_name, out_codec_name)) {
                    encoder_name = "native";
                }
            }

            av_log(NULL, AV_LOG_INFO, " encoder_name: %s ", encoder_name);
            av_log(NULL, AV_LOG_INFO, " in_codec_name: %s ", in_codec_name);
...
static int transcode(void)
{
    ...
    av_log(NULL, AV_LOG_INFO, "transcode_init\n");
    ret = transcode_init();
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "transcode_init() goto fail\n");
        goto fail;
    }
...
#if HAVE_PTHREADS
    av_log(NULL, AV_LOG_INFO, "HAVE_PTHREADS\n");
    if ((ret = init_input_threads()) < 0) {
        av_log(NULL, AV_LOG_ERROR, "init_input_threads() goto fail\n");
        goto fail;
    }
#endif
...
#if HAVE_PTHREADS
    av_log(NULL, AV_LOG_INFO, "free_input_threads()\n");
    free_input_threads();
#endif
 ...
 fail:
#if HAVE_PTHREADS
    av_log(NULL, AV_LOG_INFO, "HAVE_PTHREADS free_input_threads()\n");
    free_input_threads();
#endif
...
int ffmpegmain(int argc, char **argv)
{
    ...
    av_log(NULL, AV_LOG_DEBUG, "avcodec_register_all()\n");
    avcodec_register_all();
#if CONFIG_AVDEVICE
    av_log(NULL, AV_LOG_DEBUG, "avdevice_register_all()\n");
    avdevice_register_all();
#endif
    av_log(NULL, AV_LOG_DEBUG, "avfilter_register_all()\n");
    avfilter_register_all();
    av_log(NULL, AV_LOG_DEBUG, "av_register_all()\n");
    av_register_all();
    av_log(NULL, AV_LOG_DEBUG, "avformat_network_init()\n");
    avformat_network_init();

    av_log(NULL, AV_LOG_DEBUG, "show_banner()\n");
//    show_banner(argc, argv, options);

    av_log(NULL, AV_LOG_DEBUG, "term_init()\n");
    term_init();

    /* parse options and open all input/output files */
    ret = ffmpeg_parse_options(argc, argv);
    av_log(NULL, AV_LOG_DEBUG, "ffmpeg_parse_options()\n");
    if (ret < 0) {
        av_log(NULL, AV_LOG_DEBUG, "ffmepg_parse_options& result %d exit\n", ret);
        exit_program(1);
    }
...
//     }
    ...
    ret = transcode();
    av_log(NULL, AV_LOG_INFO, " transcode() result is %d", ret);
    if (ret < 0) {
        exit_program(1);
    }
    ti = getutime() - ti;
    ...
    if ((decode_error_stat[0] + decode_error_stat[1]) * max_error_rate < decode_error_stat[1]) {
        av_log(NULL, AV_LOG_ERROR, "transcod error\n");
        exit_program(69);
    }
